...
class HVAE(nn.Module):
    def __init__(self, input_size: int, output_size: int, symbol_library: SymbolLibrary,
                 hidden_size: Union[None, int]=None, max_height: int=8):
...
class Decoder(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, output_size: int, symbol_library: SymbolLibrary, max_height: int=8):
        ...
        self.leaf_mask = torch.zeros(len(self.symbol2index))
        for s, i in self.symbol2index.items():
            self.index2symbol[i] = s
            if self.symbol_library.get_type(s) in ["var", "const", "lit"]:
                self.leaf_mask[i] = 1.0
    ...
    def recursive_decode(self, hidden: torch.Tensor, mask: torch.Tensor, height=0) -> BatchedNode:
        prediction = F.softmax(self.h2o(hidden), dim=1)
        # Sample symbol in a given node
        if height >= self.max_height:
            prediction = prediction * self.leaf_mask
