...
from src.fasteval.library import Token, PlaceholderConstant, HardCodedConstant
...
unprotected_ops = [
    ...
    Token(power, "pow", arity=2, complexity=4)
]
...
def create_tokens(n_input_var, function_set, protected, decision_tree_threshold_set=None):
    ...
    for i in range(n_input_var):
        token = Token(name=variable_names[i], arity=0, complexity=1,
